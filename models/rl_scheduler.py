...
def train_rl(workload_train: np.ndarray, predictions_train: np.ndarray,
    ...
    model = PPO("MlpPolicy", env, verbose=1, device="cpu")  # MLP runs faster on CPU
...
def evaluate_rl(model, workload_test: np.ndarray,
    ...
    Captures pre-step queue state for accurate response-time calculation.
    ...
    while not done:
        ...
        # Response time: avg wait in queue + execution time
        # Wait ≈ (pre-step queue depth / service rate) × execution time
